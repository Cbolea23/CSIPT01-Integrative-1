...
function showReceiptModal(type, amount, newBalance) {
    ...
    let receiptModal = new bootstrap.Modal(modalElement, {
        backdrop: 'static',
        keyboard: false
    });
    receiptModal.show();
...
async function makeTransfer() {
    let targetBank = document.getElementById("target-bank").value;
    ...
    let endpoint = targetBank === "internal" ? "transfer/" : "external-transfer/";
    let payload = { destination_account: destAcc, amount: amount };
    
    if (targetBank !== "internal") { payload.target_bank = targetBank; }
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById("target-bank").value = "internal";
            showReceiptModal(targetBank === "internal" ? "transfer" : `Transfer to ${targetBank}`, amount, data.new_balance);
            loadAccount();
        } else {
            alert("Error: " + data.error);
        }
